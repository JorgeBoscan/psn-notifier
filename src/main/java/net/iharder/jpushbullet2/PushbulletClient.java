...
public class PushbulletClient {
    ...
    private static final Logger LOGGER = Logger.getLogger(PushbulletClient.class);
        ...
        try {
            ...
            websocketSession = websocketClient.connectToServer(new Endpoint() {
                ...
                public void onError(Session session, Throwable thr) {
                    LOGGER.info("Websocket session error: " + thr.getLocalizedMessage());
                }   // end onClose
...
/* ********   D E V I C E S   ******** */
    ...
    public String setDeviceNickname(String iden, String nickname) throws PushbulletException {
        return doHttpPost(API_DEVICES_URL + "/" + iden,
...
/* ********   C O N T A C T S   ******** */
     ...
     */
    public List<Contact> getContacts() throws PushbulletException {
        String result = doHttpGet(API_CONTACTS_URL);
        ContactList contactList = JsonHelper.fromJson(result, ContactList.class);
...
/* ********   S E N D I N G   P U S H E S   ******** */
    ...
    /**
     * Sends a push to the specified Pushbullet device or user.
     * @param push push message
     * @return A Java Future object related to the completion of the task
     * @throws PushbulletException if there is a communication or other error
     */
    public String sendPush(SendPush push) throws PushbulletException {
        SendPush sendPush;
        if (push instanceof FilePush) {
            FilePush filePush = (FilePush) push;
            UploadRequest uploadRequest = requestUpload(filePush.getFile());
            uploadFile(filePush.getFile(), uploadRequest);

            sendPush = new FilePush.FilePushBuilder()
                    .body(filePush.getBody())
                    .deviceIden(filePush.getDevice_iden())
                    .email(filePush.getEmail())
                    .fileName(uploadRequest.file_name)
                    .fileType(uploadRequest.file_type)
        ...
        String body = gson.toJson(sendPush);

        return doHttpPostTest(API_PUSHES_URL, body);
    }

    /**
     * Sends a push to the specified Pushbullet device or user on a separate thread.
     *
     * @param push  push message
     * @param async optional callback
     * @return A Java Future object related to the completion of the task
     */
    public Future<String> sendPushAsync(final SendPush push, final Callback<String> async) {
...
/* ********   I N T E R N A L   ******** */
     ...
     * @param upReq upload request data.
     * @throws PushbulletException if there is a communication or other error.
     */
    private void uploadFile(File file, UploadRequest upReq) throws PushbulletException {
    ...
    /**
     * Helper method for requesting upload.
     * @param file file to upload.
     * @return Upload data.
     * @throws PushbulletException if there is a communication or other error.
     */
    private UploadRequest requestUpload(File file) throws PushbulletException {
    ...
    protected String doHttpPostTest(String url, String body) throws PushbulletException {
        HttpPost post = new HttpPost(url);
        try {
...
/* ********   J S O N   H E L P E R   C L A S S E S   ******** */
    ...
    protected static class ContactList {
